...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function verifyAdminAndFetch() {
      ...
      if (!profile || profile.role !== 'admin') {
        router.push("/dashboard/staff"); // Kicks staff out of admin page
        return;
      }
  ...
  async function fetchStats() {
    ...
    if (tickets) {
      ...
      setHistoryData([
        { time: '8am', tickets: 12 }, { time: '10am', tickets: 45 },
        { time: '12pm', tickets: 30 }, { time: '2pm', tickets: 65 },
        { time: '4pm', tickets: 20 },
      ]);
    }
  }
  ...
  const resetQueue = async () => {
    if (confirm("Reset everything for tomorrow? (Numbering starts at #1)")) {
      ...
      await supabase.rpc('reset_ticket_sequence'); // Resets DB counter
      fetchStats();
    }
  };

  if (loading) return <div className="p-10 font-bold uppercase tracking-widest text-blue-900">Checking Admin Credentials...</div>;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-[#2563eb] p-8 rounded-[2rem] text-white shadow-xl">
            <p className="text-[10px] font-black uppercase tracking-widest opacity-80 mb-2">Currently Waiting</p>
            <p className="text-7xl font-black">{stats.waiting}</p>
          </div>
          <div className="bg-[#eab308] p-8 rounded-[2rem] text-white shadow-xl">
            <p className="text-[10px] font-black uppercase tracking-widest opacity-80 mb-2">Being Served</p>
            <p className="text-7xl font-black">{stats.serving}</p>
          </div>
          <div className="bg-[#22c55e] p-8 rounded-[2rem] text-white shadow-xl">
            ...
            <p className="text-7xl font-black">{stats.done}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 bg-white p-10 rounded-[3rem] shadow-sm border border-slate-200">
            <h2 className="text-xl font-black text-[#1e293b] uppercase mb-8 border-b pb-4">Live Display Control</h2>
            <div className="space-y-6">
              ...
              <button onClick={updateMonitors} className="w-full bg-[#1e293b] text-white p-5 rounded-2xl font-black uppercase tracking-widest hover:opacity-90 transition shadow-lg">Update All Monitors</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-200">
              <h2 className="text-xl font-black text-[#1e293b] uppercase mb-8">Hourly Traffic Volume</h2>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={historyData}>
                    ...
                    <Bar dataKey="tickets" fill="#2563eb" radius={[10, 10, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-[#0f172a] p-10 rounded-[3rem] text-white shadow-xl">
              ...
              <p className="text-sm text-slate-400 mb-6 font-bold uppercase tracking-tight">Daily Reset: Clears all data and restarts count to #1.</p>
              <button onClick={resetQueue} className="bg-red-600 hover:bg-red-700 text-white px-10 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition shadow-lg active:scale-95">Confirm Full System Reset</button>
            </div>
